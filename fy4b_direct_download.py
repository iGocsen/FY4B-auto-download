...
流程:
1. 查找 截止：{B1}.txt 文件，提取过期时间
2. 当前时间 > B1 → 先读取旧txt批量下载 → 再更新 Excel (B162→B2), 生成新的待导入txt
3. 当前时间 <= B1 → 直接下载 (跳过已有文件)
"""
...
def find_txt_file():
    """查找待导入txt文件，返回 (Path, datetime) 或 (None, None)"""
...
def update_excel():
    """
    更新 Excel (B162→B2, 重算) 并生成新的待导入txt。
...
def download_links(links):
    ...
    log(f"[DOWNLOAD] 共 {total} 个文件, 目标: {DOWNLOAD_DIR}")
...
def main():
    ...
    if b1_dt:
        ...
        need_update = True
        log("[CHECK] 未找到待导入txt文件，需要更新 Excel")

    # 2. 存在旧 txt → 先下载旧批次（无论是否过期都要先下载）
    if txt_path and txt_path.exists():
        old_links = read_txt_links(txt_path)
        log(f"[DOWNLOAD] 旧批次: {txt_path.name} ({len(old_links)} 个链接)")
        ok, skip, fail = download_links(old_links)
        log(f"[DONE] 旧批次 | 新下载: {ok} | 跳过: {skip} | 失败: {fail}")

    # 3. 过期 → 下载完旧批次后再更新 Excel，下载新批次
    if need_update:
        old_txt_path = txt_path
        new_txt_path = update_excel()
        if not new_txt_path:
            log("[FAIL] Excel 更新失败，退出")
            return False

        # 删除旧 txt
        if old_txt_path and old_txt_path.exists() and old_txt_path != new_txt_path:
            old_txt_path.unlink()
            log(f"[DELETE] 旧文件: {old_txt_path.name}")

        # 下载新批次
        new_links = read_txt_links(new_txt_path)
        log(f"[DOWNLOAD] 新批次: {new_txt_path.name} ({len(new_links)} 个链接)")
        ok, skip, fail = download_links(new_links)
        log(f"[DONE] 新批次 | 新下载: {ok} | 跳过: {skip} | 失败: {fail}")
    elif not txt_path:
        log("[FAIL] 无待导入文件且无旧txt，退出")
        return False
